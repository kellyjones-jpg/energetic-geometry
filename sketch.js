  ...
  switch (warpStyle) {
    case 'linear':
      for (let i = -size; i <= size; i += 8) {
        ...
        // Black glow aura layers (biggest, lowest alpha first)
        for (let glow = 4; glow >= 1; glow--) {
          let alphaGlow = 40 / glow + 20 * sin(frameCount * 0.4 + i * glow);
          pg.stroke(0, 0, 0, alphaGlow);
          pg.strokeWeight(3 + glow * 4);
          pg.line(
            i + jitterX + glow,
        ...
        // Main glowing line pulsing in alpha and strokeWeight
        let alphaMain = 200 + 80 * sin(frameCount * 0.6 + i);
        let strokeW = 4 + sin(frameCount * 0.7 + i) * 1.5;
      ...
      // Horizontal grid lines with subtle flicker
      pg.stroke(255, 60 + 40 * sin(frameCount * 0.2));
      pg.strokeWeight(0.6);
      for (let yOff = -size; yOff <= size; yOff += 10) {
        let horizOffset = sin(frameCount * 0.07 + yOff) * 2;
    ...
    case 'symmetric':
      for (let i = 0; i < size; i += 4) {
        ...
        // Black glow aura layers with flicker + pulsating strokeWeight
        for (let glow = 4; glow >= 1; glow--) {
        ...
        // Colored glow layers behind main line
        for (let glow = 4; glow >= 1; glow--) {
    ...
    case 'radial':
      for (let r = 12, idx = 0; r < size; r += 10, idx++) {
        ...
        // Colored glow rings behind main ellipse
        for (let glow = 3; glow >= 1; glow--) {
        ...
        // Tiny noisy points on perimeter with flicker
        let pointsCount = 12;
